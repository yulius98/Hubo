...
import { Menu, X, Sun, Moon, Search, LogOut, Languages } from 'lucide-react';
...
const MobileMenu = memo(function MobileMenu({
    mobileMenuOpen, scrollToTop, scrollToKategori, scrollToProduk, locale, setLocale, user,
}: Readonly<{
            ...
            {mobileMenuOpen && (
                ...
                >
                    <div className="flex flex-col gap-1 px-5 py-4">
                        ...
                        <button
                            type="button"
                            onClick={() => { if (!profilLoading) { setProfilLoading(true); router.visit(myprofile()); } }}
                        ...
                        <LoadingOverlay show={profilLoading} />
                        <Link href={logout()} className="flex items-center gap-3 rounded-lg px-4 py-3 text-base font-medium text-red-300 transition hover:bg-blue-800/40 hover:text-red-200">
                            <LogOut size={20} /> {t('nav.logout', locale)}
                        </Link>
                    </div>
                </motion.div>
            )}
                ...
                >
                    {kategoris.map((kategori) => (
                        <motion.div
                            ...
                            key={kategori.id}
                            onClick={() => handleKategoriClick(kategori.id)}
                            className={`min-w-40 overflow-hidden rounded-xl shadow-xl sm:min-w-50 md:min-w-60 md:rounded-2xl ${cardClass} snap-center cursor-pointer ${
                                selectedKategori === kategori.id
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="flex h-16 items-center justify-between md:h-20">
                        ...
                        <div className="hidden items-center gap-5 md:flex lg:gap-7">
                            ...
                            <span className="text-sm font-medium text-blue-100">{t('nav.hi', locale, { name: user?.name ?? 'Guest' })}</span>
